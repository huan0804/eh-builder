...
- **Cold Open** (`game/src/screens/ColdOpen.jsx`): tổng quát theo `data.coldOpen` (`timestamp`, `hesitant`, `finalLine`, `chatListEntries`) — screen chỉ render, khác biệt giữa các phần (Phần 1 do dự/1 dòng chat cũ, Phần 2 không do dự/2 dòng chat) đều đến từ data. Cờ "đã xem" trong `localStorage` (độc lập với autosave, giữ nguyên qua `RESTART`).
