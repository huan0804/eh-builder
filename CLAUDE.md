...
- **Chân dung nhân vật theo cảm xúc** (`game/src/components/CharacterPortrait.jsx`, thêm 09/2026, lấy cảm hứng Ace Attorney — quyết định thay cho hướng 3D/thế giới mở kiểu Black Myth: Wukong, không khả thi với đội 1-2 người): mỗi suspect trong `characters` có thể khai báo `portraits: { emotion: 'path/to.png' }`; mỗi node hội thoại có field `emotion` (vd `neutral`/`nervous`/`defensive`/`confession`). `Investigation.jsx` hiện portrait của suspect đang phỏng vấn, đổi theo `emotion` của node hiện tại. Ảnh nạp qua `import.meta.glob('../assets/portraits/**/*.png')` — CHƯA có ảnh thật nào (chỉ mới xong cho case1 ở mức data + engine), tự fallback về khung màu + icon cảm xúc khi ảnh chưa tồn tại hoặc tải lỗi, không bao giờ phá layout. **Tạo ảnh theo `docs/portrait-prompts.md`** (style guide + prompt chi tiết cho từng nhân vật/cảm xúc, người dùng tự chạy qua công cụ AI image bên ngoài rồi lưu vào `game/src/assets/portraits/<suspectId>/<emotion>.png` — không cần sửa code gì thêm sau khi thêm ảnh).
...
- `game/src/screens/` — màn hình dùng chung cho mọi phần: `ColdOpen`, `Prologue`, `Chapter1`, `Investigation`, `ThinkingBoard`, `Epilogue`.
...
- Tạo ảnh thật cho chân dung nhân vật (9 ảnh Phần 1: Khang/Chi/Đức × 2-3 trạng thái) theo `docs/portrait-prompts.md` — hiện engine đã sẵn sàng nhưng đang hiện fallback khung màu/icon vì chưa có ảnh. Sau đó mở rộng `portraits`/`emotion` cho case2.js/case3.js.
