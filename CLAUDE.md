...
- **Chân dung nhân vật theo cảm xúc** (`game/src/components/CharacterPortrait.jsx`, thêm 09/2026, lấy cảm hứng Ace Attorney — quyết định thay cho hướng 3D/thế giới mở kiểu Black Myth: Wukong, không khả thi với đội 1-2 người): mỗi character trong `characters` có thể khai báo `portraits: { emotion: 'path/to.svg' }`; mỗi node hội thoại (Investigation) và mỗi dòng `prologue.lines` (`speakerId` + `emotion`) chỉ định cảm xúc hiển thị (`neutral`/`nervous`/`defensive`/`confession`/`thinking`/`worried`...). `Investigation.jsx` hiện portrait suspect đang phỏng vấn cạnh tên; `Prologue.jsx` hiện portrait nhỏ (`.portrait-small`) cạnh MỖI dòng thoại, đổi theo người đang nói — không chỉ Investigation mới có portrait. Ảnh nạp qua `import.meta.glob('../assets/portraits/**/*.{png,jpg,jpeg,svg}')`, tự fallback về khung màu + icon cảm xúc khi ảnh chưa tồn tại hoặc tải lỗi. **Phần 1 đã có 12 ảnh** (Khang/Chi/Đức/Lam/Cô Hạnh × các trạng thái) — nhưng là **SVG vector tự vẽ bằng code**, KHÔNG phải ảnh AI-art photorealistic (phiên không có công cụ tạo ảnh AI khả dụng lúc đó). Chất lượng thấp hơn ảnh AI theo đúng prompt đã viết ở `docs/portrait-prompts.md` — nên thay thế bằng ảnh AI thật khi có điều kiện (chỉ cần ghi đè đúng path, xem mục 4 của file đó).
...
- Thay ảnh SVG tự vẽ (Phần 1: portrait Khang/Chi/Đức/Lam/Cô Hạnh + ảnh nền title/phòng CLB) bằng ảnh AI thật theo `docs/portrait-prompts.md` khi có công cụ tạo ảnh AI khả dụng — chất lượng SVG hiện tại chỉ là giải pháp tạm, thấp hơn nhiều so với ảnh AI-art theo đúng prompt.
- Mở rộng `portraits`/`emotion`/`background` cho case2.js/case3.js (Investigation + Prologue/Chapter1 của 2 phần đó chưa có portrait/scene bg, chỉ mới làm Phần 1).
